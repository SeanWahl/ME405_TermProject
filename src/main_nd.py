...
#             line += "\r\n"
    ...
    def get_hotColumn(self):
        ...
        pix2deg = 0.805 #[degree/pixel]
        deg2ticks = 45.511 #[ticks/deg]
        
        if maxIdx >= 16: # if person is in right half
            maxIdxnew = maxIdx - 16
            degree_move = pix2deg*maxIdxnew # Degrees to move right
            ticks_move = degree_move*deg2ticks
            print(f'Move motor {ticks_move} right')
            #CLController(0.1, ticks_move)
            
        else: # if person is in left half
            maxIdxnew = 16 - maxIdx
            degree_move = pix2deg*maxIdxnew # Degrees to move right
            ticks_move = degree_move*deg2ticks
            print(f'Move motor {ticks_move} left')
            #CLController(0.1,ticks_move)
